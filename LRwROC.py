...
def model(data_train, data_valid, data_test):
    ...
    pos_ind = np.where(lr_orig_panel19.classes_ == data.favorable_label)[0][0]
    dataset_orig_train_pred = data.copy(deepcopy=True)
    dataset_orig_train_pred.labels = y_train_pred
    
    #validate model
    dataset_orig_valid_pred = dataset_valid.copy(deepcopy=True)
    ...
    #evaluate ROC Model on Validation Set
